...
window.actionEvents = {
    ...
    'click .table-apps-remove': function (e, value, row, index) {
        var url = urls["API_CLIENTS_DELETE"]["url"].format({"id": row["clientId"],"definition":"RegisterFintech"})
    ...
    $("#modal-apps").on('show.bs.modal', function(e) {
        ...
        var url = urls["API_CLIENTS_DETAIL"]["url"].format({"id":rowid, "definition":"RegisterFintech"});
   ...
   $("#new-app-btn").click(function(e){
	...
	var url = urls["API_CLIENTS_REGISTER"]["url"].format({"definition": "RegisterFintech"});
	var headers = {"Accept": "application/json","Content-type": "application/json"};
